



$(document).ready(function(){



    $("#combo1").click(function() {
        $("#combo1-lista").toggle();
    });

    $("#combo2").click(function() {
        $("#combo2-lista").toggle();
    });

    $("#combo3").click(function() {
        $("#combo3-lista").toggle();
    });

    $("#combo4").click(function() {
        $("#combo4-lista").toggle();
    });
    
    $("#combo5").click(function() {
        $("#combo5-lista").toggle();
    });

    $("#combo6").click(function() {
        $("#combo6-lista").toggle();
    });
    $("#combo7").click(function() {
        $("#combo7-lista").toggle();
    });
    $("#temaki1").click(function() {
        $("#temakilista1").toggle();
    });
    $("#temaki2").click(function() {
        $("#temakilista2").toggle();
    });
    $("#temaki3").click(function() {
        $("#temakilista3").toggle();
    });
    $("#temaki4").click(function() {
        $("#temakilista4").toggle();
    });
    $("#temaki5").click(function() {
        $("#temakilista5").toggle();
    });
    $("#mostrarCombos").click(function() {
        $("#combos").toggle();
    });
    $("#mostrarTemaki").click(function() {
        $("#temakis").toggle();
    });
    $("#mostrarBebidas").click(function() {
        $("#bebidas").toggle();
    });
    $("#coca1l").click(function() {
        $("#cocalista1").toggle();
    });
    $("#coca2l").click(function() {
        $("#cocalista2").toggle();
    });
    $("#cocalata").click(function() {
        $("#cocalata2").toggle();
    });
    $("#guarana").click(function() {
        $("#guarana2").toggle();
    });
    $("#vinho1").click(function() {
        $("#vinhogalioto").toggle();
    });
    
    $("#vinho2").click(function() {
        $("#vinhoconcha").toggle();
    });
    $("#vinho3").click(function() {
        $("#vinhosanta").toggle();
    });
    
    



});
